...
class GaussianModel:
    ...
    def reduce_opacity(self, rate):
        opacities_new = self.inverse_opacity_activation(self.get_opacity * rate)
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
    ...
    def final_prune(self, final_budget):
        imp_score = self.get_opacity.squeeze()
    ...
    def update_opacity_lr(self, rate):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
